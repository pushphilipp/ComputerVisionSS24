...
def render_correspondence(image1: t_image, points1: t_points, image2:t_image, points2:t_points, color:t_color=[0, 255, 0],
    ...
    points1, points2 = points1.astype(int), points2.astype(int)
...
def save_images(images: t_image_list, filenames: t_str_list, **kwargs) -> None:
    ...
    for n in range(len(images)):
        file_name = filenames[n]
        _create_directory(os.path.dirname(file_name))
        if images[n].dtype == np.uint8:
            cv2.imwrite(file_name, images[n], **kwargs)
        elif images[n].dtype in [np.float32, np.float64, bool]:
            image = np.clip(images[n], 0, 1)
            image = (image * 255.).astype(np.uint8)
            cv2.imwrite(file_name, image, **kwargs)
        else:
            raise ValueError
