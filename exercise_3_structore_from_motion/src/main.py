...
def main(parameters):
    ...
    show_images([render_correspondence(img1, points1, img2, points2,
                                               scale=p.correspondence_scale,
                                               thickness=3)], ["Correspondences"])
    output_path = os.path.join(os.getcwd(), p.output_path)
    save_images([render_correspondence(img1, points1, img2, points2,
                                               scale=p.correspondence_scale,
                                               thickness=3)],
                    [os.path.join(output_path, "Correspondences_before_ransac.png")])
    ...
    show_images([render_correspondence(img1, inlier_points1, img2, inlier_points2,
                                               scale=p.correspondence_scale,
                                               thickness=2)], ["Correspondences"])
    save_images([render_correspondence(img1, inlier_points1, img2, inlier_points2,
                                               scale=p.correspondence_scale,
                                               thickness=2)],
                    [os.path.join(output_path, "Correspondences_after_ransac.png")])
    ...
    wps = triangulate_all_points(View1, View2, intrinsic_camera_params, inlier_points1, inlier_points2)
    ...
    colors= [[0, 0, 1.], [1, 0, 0.]]
    np_lineset = create_camera_lineset(wps, cameras, colors)
    o3d.visualization.draw_geometries([o3d_from_drawable(dr) for dr in [np_lineset, wps]])
