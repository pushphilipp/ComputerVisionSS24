...
import numpy as np
import cv2
from utils import extract_features, filter_and_align_descriptors, compute_homography, ransac, translate_homographies
...
def test_features(img_count_bbox):
    ...
    points, descriptors = extract_features(img, 500)
...
def test_filter_and_align_descriptors(translation):
    ...
    points, descriptors = extract_features(image1, 18)
    translated_points, translated_descriptors = extract_features(image2, 18)
    points, translated_points = filter_and_align_descriptors((points, descriptors), (translated_points, translated_descriptors), similarity_threshold=.7)
...
def test_translate_homography(translation):
    ...
    homographies = {("src", "dst"): compute_homography(points1, points2)}
    points = np.random.rand(1000, 2) * 100
    translated_homographies = translate_homographies(homographies, dx=translation[0], dy=translation[1])
