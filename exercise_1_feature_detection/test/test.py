...
sys.path.append(".")
from utils import compute_harris_response, detect_corners, detect_edges
...
def test_harris(img_mean_dev):
    ...
    R, _, _, _, _, _ = compute_harris_response(img, 0.06)
...
def test_harris_vectorization():
    lp = LineProfiler()
    lp.add_function(compute_harris_response)
    lp.runcall(compute_harris_response, float_image, 0.06)
    stats = lp.get_stats()
    assert all([count == 1 for _, count, _ in list(stats.timings.values())[0]])


@pytest.mark.parametrize("img_pointcount", [(image, 0), (image1, 4), (image2,  8)])
def test_points(img_pointcount):
    img, pointcount = img_pointcount[0][:, :, 0].astype(np.float32)/255., img_pointcount[1]
    R, _, _, _, _, _ = compute_harris_response(img, 0.06)
    points = detect_corners(R)
    assert len(points[0]) == len(points[1]) == pointcount


def test_point_vectorization():
    lp = LineProfiler()
    lp.add_function(detect_corners)
    lp.runcall(detect_corners, float_image, 0.1)
    stats = lp.get_stats()
...
def test_edges(img_masklength):
    ...
    R, _, _, _, _, _ = compute_harris_response(img, 0.06)
    edges = detect_edges(R)
    assert edges.sum() in edge_sum


def test_edges_vectorization():
    lp = LineProfiler()
    lp.add_function(detect_edges)
    lp.runcall(detect_edges, float_image, 0.1)
    stats = lp.get_stats()
